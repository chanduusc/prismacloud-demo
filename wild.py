...
unique_bucketname = 'panpartnersa-' + str(uuid.uuid4().hex)+ '-' + str(datetime.datetime.now().time()).replace(':', '-').replace('.', '-')
...
class MyServer(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.wfile.write(bytes("<p>File name: %s</p>" % unique_bucketname, "utf-8"))
